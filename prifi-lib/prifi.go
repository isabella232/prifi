...
func NewPriFiClient(doLatencyTest bool, dataOutputEnabled bool, dataForDCNet chan []byte, dataFromDCNet chan []byte, doReplayPcap bool, pcapFolder string, schedulerStrategy string, msgSender net.MessageSender) *PriFiLibInstance {
	msw := newMessageSenderWrapper(msgSender)
	c := client.NewClient(doLatencyTest, dataOutputEnabled, dataForDCNet, dataFromDCNet, doReplayPcap, pcapFolder, schedulerStrategy, msw)
	p := &PriFiLibInstance{
